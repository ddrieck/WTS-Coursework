...
if (($page_base == 'registration')&&($_SERVER['REQUEST_METHOD'] == 'POST')&&(empty($_POST['username'])||empty($_POST['password'])||empty($_POST['repeatpassword'])||empty($_POST['firstname'])||empty($_POST['lastname'])||empty($_POST['email'])||empty($_POST['phone'])||empty($_POST['avatar']))) { $status = '<p id="error">Please complete the registration form.</p>'; } 
		//perform password match check
		else if ((($page_base == 'registration')&&($_SERVER['REQUEST_METHOD'] == 'POST'))&&($_POST['password'] != $_POST['repeatpassword'])) { $status = '<p id="error">Your passwords do not match.</p>'; } 
			else if (($page_base == 'registration')&&($_SERVER['REQUEST_METHOD'] == 'POST')&&(isset($_POST['username'])&&isset($_POST['password'])&&isset($_POST['repeatpassword'])&&isset($_POST['firstname'])&&isset($_POST['lastname'])&&isset($_POST['email'])&&isset($_POST['phone'])&&isset($_POST['avatar'])))
